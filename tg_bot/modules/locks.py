    ...
    if can_delete(chat, bot.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("قفل شد {}!".format(args[0]))
                ...
                message.reply_text(
                    "چه چیزی را میخوای قفل کنی از این دستور برای مشاهده لیست استفاده کن /locktypes")

    else:
        message.reply_text("من ادمین نیستم به من دسترسی دهید.")
    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("باز شد {}!".format(args[0]))
                return "<b>{}:</b>" \
                       "\n#UNLOCK" \
                       "\n<b>Admin:</b> {}" \
                ...
                message.reply_text(
                    "چه چیزی را میخواهی باز کنی از این دستور استفاده کن /locktypes")

        else:
            bot.sendMessage(chat.id, "چه چیزی را میخوای باز کنی؟")
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, bot.id):
                            message.reply_text(
                                "من یک ربات میبینم بهت میگم نذار اینا جوین شن ولی گوش نمیکنی!")
                            return

                        chat.kick_member(new_mem.id)
                        message.reply_text(
                            "فقط ادمین ها میتونن ربات به گروه بیارن! گمشو بیرون!.")
            else:
                try:
                    message.delete()
...
def build_lock_message(chat_id):
    ...
    if not (locks or restr):
        res = "این ها در گپ قفل نیستند."
    else:
        res = "این ها در گپ قفل هستند:"
        if locks:
            res += "\n - sticker = `{}`" \
                   "\n - audio = `{}`" \
                   "\n - voice = `{}`" \
...
__help__ = """
 - /locktypes: لیست قفل های کنونی

*فقط ادمین ها:*
 - /lock <type>: قفل کردن آیتم مورد نظر
 - /unlock <type>: از قفل درآوردن آیتم مورد نظر
 - /locks: لیست چیزهایی که در گروه قفل میباشند

از قفل ها می توان برای محدود کردن کاربران یک گروه استفاده کرد.
به عنوان مثال:
قفل کردن آدرسها باعث حذف خودکار همه پیامها با آدرسها می شود ، قفل کردن برچسب ها همه را حذف می کند
برچسب ها و غیره
قفل کردن ربات ها از اضافه کردن ربات ها به گپ توسط غیر از ادمین ها جلوگیری کرد."""
