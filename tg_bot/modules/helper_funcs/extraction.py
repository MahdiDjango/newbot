...
def extract_user_and_text(message: Message, args: List[str]) -> (Optional[int], Optional[str]):
    ...
    if entities and ent and ent.offset == len(message.text) - len(text_to_parse):
        ...
        if not user_id:
            message.reply_text(
                "من کاربر را در دیتابیسم ندارم اگر میخوای داشته باشم پیامی ازش ریپلای کن.")
            return None, None
    ...
    try:
        ...
        if excp.message in ("User_id_invalid", "Chat not found"):
            message.reply_text(
                "به نظر نمی رسد قبلاً با این کاربر تعامل داشته باشم - لطفاً پیامی را ازآن ارسال کنید)")
        else:
