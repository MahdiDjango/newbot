...
CURRENT_WARNING_FILTER_STRING = "<b>فیلتر های شامل اخطار:</b>\n"


# Not async
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    if is_user_admin(chat, user.id):
        message.reply_text("اوه؛ ادمین ها نمیتونن اخطار بگیرن!")
        return ""
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # kick
            ...
            reply = "{} اخطار, {} با این تعداد گورتو گم میکنی!".format(
                limit, mention_html(user.id, user.first_name))

        else:  # ban
            chat.kick_member(user.id)
            reply = "{} اخطار, {} با این تعداد گورتو گم میکنی ولی میتونی برگردی!".format(
                limit, mention_html(user.id, user.first_name))
        ...
        reply = "{} دارد {}/{}اخطار!".format(mention_html(user.id, user.first_name), num_warns,
                                             limit)
        if reason:
...
def button(bot: Bot, update: Update) -> str:
    ...
    if match:
        ...
        if res:
            ...
            update.effective_message.edit_text(
                "این شخص درحال حاضر اخطار ندارد.".format(
                    mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML)
...
def warn_user(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id:
        ...
        message.reply_text("هیچ کاربری مشخص نشده است!")
    return ""
...
def reset_warns(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id:
        ...
        message.reply_text("اخطار ها ریست شد!")
...
def warns(bot: Bot, update: Update, args: List[str]):
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            text = "این شخص این تعداد اخطار دارد {}/{}, به دلایل زیر:".format(
                num_warns, limit)
            ...
            update.effective_message.reply_text(
                "این شخص این تعداد اخطار دارد {}/{},  اما دلیلی برای آنها ذکر نشده.".format(num_warns, limit))
    else:
        update.effective_message.reply_text(
            "این کاربر هیچ اخطاری دریافت نکرده است!")
...
def add_warn_filter(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text(
        "اخطار داده شده برای  '{}'!".format(keyword))
...
def remove_warn_filter(bot: Bot, update: Update):
    ...
    if not chat_filters:
        msg.reply_text("هیچ فیلتر اخطار دهنده ای اینجا نیست!")
        return

    for filt in chat_filters:
        if filt == to_remove:
            sql.remove_warn_filter(chat.id, to_remove)
            msg.reply_text("یس؛ من اخطار دادن را برای آن متوقف میکنم.")
            raise DispatcherHandlerStop

    msg.reply_text(
        "هیچ فیلتر اخطار دهنده ای نیست از این دستور استفاده کن تا همه ی آنها را ببینی /warnlist.")
...
def list_warn_filters(bot: Bot, update: Update):
    ...
    if not all_handlers:
        update.effective_message.reply_text("هیچ فیلتر اخطار دهنده ای نیست!")
        return
...
def set_warn_limit(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if args:
        if args[0].isdigit():
            if int(args[0]) < 3:
                msg.reply_text("حداقل اخطار ۳ می باشد!")
            else:
                sql.set_warn_limit(chat.id, int(args[0]))
                msg.reply_text(
                    "تعداد اخطار ها به این عدد تغییر کرد {}".format(args[0]))
                return "<b>{}:</b>" \
                       "\n#SET_WARN_LIMIT" \
                       "\n<b>Admin:</b> {}" \
            ...
            msg.reply_text("به من یک عدد بده!")
    else:
        limit, soft_warn = sql.get_warn_setting(chat.id)

        msg.reply_text("حداقل اخطار {}".format(limit))
    return ""
...
def set_warn_strength(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            msg.reply_text("اخطار زیاد باعث شد گمشی از اینجا!")
            return "<b>{}:</b>\n" \
                   "<b>Admin:</b> {}\n" \
            ...
            msg.reply_text(
                "اخطار زیاد باعث شد گمشی ولی بعدا میتونی برگردی.")
            return "<b>{}:</b>\n" \
                   "<b>Admin:</b> {}\n" \
            ...
            msg.reply_text("به من این دستورات بده on/yes/no/off!")
    else:
        limit, soft_warn = sql.get_warn_setting(chat.id)
        if soft_warn:
            msg.reply_text("اخطار ها باعث kick شدن میشود.",
                           parse_mode=ParseMode.MARKDOWN)
        else:
            msg.reply_text("اخطار ها باعث ban شدن میشود.",
                           parse_mode=ParseMode.MARKDOWN)
    return ""
...
def __chat_settings__(chat_id, user_id):
    ...
    return "This chat has `{}` warn filters. It takes `{}` warns " \
           "before the user gets *{}*.".format(num_warn_filters,
                                               limit, "kicked" if soft_warn else "banned")


__help__ = """
 - /warns <userhandle>: نمایش اخطار های کاربر ودلیل آنها
 - /warnlist: لیست تمام فیلترهای هشدار دهنده فعلی

*فقط ادمین ها:*
 - /warn <userhandle>: به کاربر هشدار دهید. پس از 3 اخطار ، کاربر از عضو شدن در این گروه منع خواهد شد
 - /resetwarn <userhandle>: پاک کردن اخطار های کاربر
 - /addwarn <keyword> <reply message>: یک فیلتر هشدار بر روی یک کلمه کلیدی خاص تنظیم کنید. اگر میخواهید کلمه کلیدی خود یک جمله باشد آن را میان نقل  بگذارید
 - /nowarn <keyword>: فیلتر هشدار را متوقف کنید
 - /warnlimit <num>: حد هشدار را تعیین کنید
 - /strongwarn <on/yes/off/no>: اگر روشن باشد ، بیش از حد هشدار منجر به ممنوعیت می شودو در غیر اینصورت kick میشود
"""
...
RESET_WARN_HANDLER = CommandHandler(["resetwarn", "resetwarns"],
                                    reset_warns, pass_args=True, filters=Filters.group)
...
WARN_LIMIT_HANDLER = CommandHandler("warnlimit", set_warn_limit,
