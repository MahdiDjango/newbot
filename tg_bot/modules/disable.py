...
if is_module_loaded(FILENAME):
    ...
    def disable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text(" این دستور غیرفعال شد`{}`".format(disable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("این دستور را نمیتوان غیرفعال کرد!")

        else:
            update.effective_message.reply_text("چه دستوری غیرفعال کنم؟")
    ...
    def enable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text("این دستور فعال شد`{}`".format(enable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("مطمعنی غیرفعاله؟")

        else:
            update.effective_message.reply_text("چ دستوری را فعال کنم؟")
    ...
    def list_cmds(bot: Bot, update: Update):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text("این دستورات میتوانند غیرفعال شوند:\n{}".format(result),
                                                parse_mode=ParseMode.MARKDOWN)
        else:
            update.effective_message.reply_text("هیچ دستوری غیرفعال نیست.")
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        if not disabled:
            return "هیچ دستوری غیرفعال نیست!"

        result = ""
        for cmd in disabled:
            result += " - `{}`\n".format(escape_markdown(cmd))
        return "این دستورات غیرفعال هستند:\n{}".format(result)
    ...
    __mod_name__ = "غیرفعال سازی دستورات"

    __help__ = """
 - /cmds: وضعیت فعلی دستورات غیرفعال شده را بررسی کنید

*فقط ادمین ها:*
 - /enable <cmd name>: فعال کردن آن دستور
 - /disable <cmd name>: غیرفعال کردن آن دستور
 - /listcmds: تمام دستورات قابل تغییر را فهرست کنید
    """
